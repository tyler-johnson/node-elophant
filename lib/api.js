...
(function() {
  var ELOPHANT_BASE, ELOPHANT_REGIONS, Summoner, Team, async, buildOptionsCallback, buildURL, callAPI, fixMethod, nconf, path, qs, request, _,
    __slice = [].slice;
  ...
  */
  ...
  _.each(["mastery_pages", "rune_pages", "recent_games", "leagues", "in_progress_game_info"], function(method) {
    var fnc;
  ...
  _.each(["team", "find_team"], function(method) {
    var fnc;
    fnc = function(arg, o, cb) {
  ...
  module.exports.summoner_team_info = function(sid, o, cb) {
    var callback, url, _ref;
    _ref = buildOptionsCallback(o, cb), o = _ref[0], cb = _ref[1];
    _.defaults(o, {
      complex: false
    ...
    if (o.complex) {
      callback = function(err, data) {
        if (err) {
          return cb(err);
        } else if (_.isObject(data) && _.isArray(data.teamsSummary)) {
          return cb(null, _.map(data.teamsSummary, function(team) {
